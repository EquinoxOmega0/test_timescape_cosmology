...
double precision :: dist,ts_modificator,hxsum,hysum,hzsum,hmsum,sidelength
...
! get length of file
OPEN(50,file='halos/loghalo'//TRIM(mn)//'_sorted.txt')
io_err=0
n_h=0
DO WHILE (io_err==0) 
READ(50,*,iostat=io_err) 
...
DO WHILE ((n_active_old.NE.n_active).AND.(itercount<10))

itercount=itercount+1

n_active_old=n_active

DO i=1,n_h
IF (fi_active(i)) THEN
IF (changed_last(i)) THEN
WRITE(*,*) i,n_active,uuu
! hr2(i)=h_r(i)**2

 changed_last(i)=.FALSE.
...
logmh=LOG10(hm(i))
logmfi=solution_first(1)*(logmh**2)+solution_first(2)*logmh+solution_first(3)
hm(i)=10.D0**logmfi
h_r(i)=(3.D0/(4.D0*PI)*hm(i)/(rho_crit*ts_modificator))**(1.D0/3.D0)
...
logmfi=solution_final(1)*(logmh**2)+solution_final(2)*logmh+solution_final(3)
